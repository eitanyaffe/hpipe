...
model.predict=function(ofield.x.from, ofield.x.to, ofield.y.from, ofield.y.to, index, from.fend, to.fend,
                       ...
                       filter="trans", inter.field="none", cis.threshold=100000,
                       ofields.x=c("f1", "f2"), ofields.y=c("f1", "f2"),
                       compaction.fn="none",
                       cis.mode="none", cis.binsize=0, cis.decay.table.dir="")
{
  ...
  # filtering
  command = sprintf("%s -filter %s -cis_threshold %d -trans_prior %g", command, filter, cis.threshold, prior)

  if (filter == "inter")
    command = sprintf("%s -inter_field %s", command, inter.field)
  ...
  # compaction
  if (compaction.fn != "none")
    command = sprintf("%s -compaction_fn %s", command, compaction.fn)

  # cis decay
  command = sprintf("%s -cis_mode %s", command, cis.mode)
  if (cis.mode != "none") {
    command = sprintf("%s -cis_log_step %f", command, cis.binsize)
    if (cis.decay.table.dir != "")
      command = sprintf("%s -cis_table_dir %s", command, cis.decay.table.dir)
  }
...
model.predict.split=function(tmp.dir = "/home/eitany/storage/o3c/mp_tmp/",
                             ...
                             compaction.fn="none",
                             cis.mode="none", cis.binsize=0, cis.decay.table.dir="",
                             num.splits=10, cluster=F, req.mem=2000, filter="trans", inter.field="none", cis.threshold=100000,
                             max.njobs=400, max.jobs.fn="/home/eitany/maxjobs", prior=1, fends=NULL,
                             mfields=NULL, mfields.maxvals=NULL, mfields.fns=NULL, dtype=NULL,
                             ...
                             ofields.y=c("frag_len_bin","frag_gc_bin"), from.vals.y=c(1,1), to.vals.y=c(5,5), Rcall)
{
  ...
  # if fend breaks are defined we have all tasks running on all output bins
  # and split the fend space
  #fends = NULL
  # !!! should also allow to split by fend space
  if (T || (is.null(fends) && (nbins < 1000) || filter == "id")) {
    ...
    #step = length(fends)/num.splits
    #fbreaks = round(seq(fends[1]+step, fends[length(fends)]-step, step))
  ...
  # special case for close cis and coord bins
  if (length(ofields.x) == 1 && ofields.x == "coord_bin" && length(ofields.y) == 1 && ofields.y == "coord_bin" && filter == "close_cis") {
    x = strsplit(fends.fn, "_")[[1]]
    binsize = as.numeric(strsplit(x[length(x)], "\\.")[[1]][1])
    if (is.na(binsize))
      stop("Could not extract binsize from filename")
    cis.threshold.bins = cis.threshold/binsize
    if (!all(is.element(c("coord_bin_x_from", "coord_bin_x_to", "coord_bin_y_from", "coord_bin_y_to"), names(ranges))))
      stop("ranges table does not have correct columns")
    ind = ((abs(ranges$coord_bin_x_from - ranges$coord_bin_y_from) <= cis.threshold.bins) |
           (abs(ranges$coord_bin_x_from - ranges$coord_bin_y_to)   <= cis.threshold.bins) |
           (abs(ranges$coord_bin_x_to   - ranges$coord_bin_y_from) <= cis.threshold.bins) |
           (abs(ranges$coord_bin_x_to   - ranges$coord_bin_y_to)   <= cis.threshold.bins))
    ranges = ranges[ind,]
    cat(sprintf("close cis optimization: running only %d jobs\n", sum(ind)))
  }
  ...
  for (i in 1:nrow(ranges)) {
    ...
    command = sprintf("%s %s R/model_predict.r model.predict ofields.x=\"%s\" ofield.x.from=\"%s\" ofield.x.to=\"%s\" ofields.y=\"%s\" ofield.y.from=\"%s\" ofield.y.to=\"%s\" index=%d from.fend=%d to.fend=%d fends.fn=\"%s\" log.dir=\"%s\" tmp.dir=\"%s\" binary=\"%s\" prior=%g filter=\"%s\" cis.threshold=%d compaction.fn=\"%s\" cis.mode=\"%s\" cis.binsize=%f inter.field=%s",
        ...
        binary, prior, filter, cis.threshold, compaction.fn, cis.mode, cis.binsize, inter.field)
    ...
    if (cis.decay.table.dir != "") {
      command = sprintf("%s cis.decay.table.dir=\"%s\"", command, cis.decay.table.dir)
    }
...
compute.total.counts=function(tmp.dir = "/home/eitany/storage/o3c/mp_tmp/",
                              ...
                              max.vals=c(20,5), filter="trans", cis.threshold=100000, req.mem=2000, dtype="sge", Rcall)
{
  ...
  model.predict.split(tmp.dir=tmp.dir, log.dir=log.dir, wd=wd, binary=binary,
                      fends.fn=fends.fn, ofn=ofn, cluster=cluster, fends=fends, num.splits=num.splits,
                      filter=filter, cis.threshold=cis.threshold, max.jobs.fn=max.jobs.fn,
                      ofields.x=ofields, from.vals.x=rep(1, length(ofields)), to.vals.x=max.vals,
                      ofields.y=ofields, from.vals.y=rep(1, length(ofields)), to.vals.y=max.vals, req.mem=req.mem, dtype=dtype, Rcall=Rcall)
...
compute.expected.counts=function(
    ...
    wd = "/home/eitany/storage/o3c/",
    binary = "/home/eitany/storage/o3c/bin/model_integrate",
    ifn.prefix="/home/eitany/storage/o3c/results/h1_cl_gc_1000_1M",
    cis.decay.table.dir="/home/eitany/storage/o3c/results/h1_cl_gc_1000_1M",
    fends.ifn, num.splits=100, max.jobs.fn="/home/eitany/maxjobs",
    filter="trans", inter.field="contig", cis.threshold=100000,
    model.ifn="/home/eitany/storage/o3c/results/map_len_gc.model",
    ofn="/home/eitany/storage/o3c/results/h1_cl_gc_1000_1M.e_contact",
    compaction.fn="none",
    cis.mode="none", cis.binsize=0,
    ...
    omit.x.zero=F, omit.y.zero=F, Rcall)
{
  ...
  model.predict.split(tmp.dir=tmp.dir, log.dir=log.dir, wd=wd, binary=binary,
                      fends.fn=fends.ifn, ofn=ofn, cluster=cluster, fends=fends, num.splits=num.splits,
                      filter=filter, inter.field=inter.field, cis.threshold=cis.threshold, prior=prior,
                      compaction.fn=compaction.fn, max.jobs.fn=max.jobs.fn,
                      cis.mode=cis.mode, cis.binsize=cis.binsize, cis.decay.table.dir=cis.decay.table.dir,
