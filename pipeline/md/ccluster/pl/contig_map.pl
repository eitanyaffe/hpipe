...
if ($#ARGV == -1) {
	print STDERR "usage: $0 <input dir> <contig table> <similarity mask table> <offset> <contact odir> <masked contact odir> <output>\n";
	exit 1;
}
...
my $isim = $ARGV[2];
my $offset = $ARGV[3];
my $odir_contact = $ARGV[4];
my $odir_masked = $ARGV[5];
my $ofn = $ARGV[6];
...
print STDERR "found ".scalar(@isims)." input files in: $idir\n";
...
#	print STDERR "line: $read_count\n" if ($read_count % 100000 == 0);
	...
	next if (!$f[$h{high_quality1}] || !$f[$h{high_quality2}]);
	...
	if (!defined($counts{$contig1}->{$contig2})) {
	    ...
	    $counts{$contig1}->{$contig2}->{area} = $contigs{$contig1} * $contigs{$contig2};
	}
	if (!defined($counts{$contig2}->{$contig1})) {
	    ...
	    $counts{$contig2}->{$contig1}->{area} = $contigs{$contig1} * $contigs{$contig2};
	}
...
print STDERR "generating file: $ofn\n";
open(OUT, ">", $ofn) || die $ofn;
print OUT "contig1\tcontig2\tcontacts\tmasked_contacts\tarea\n";
