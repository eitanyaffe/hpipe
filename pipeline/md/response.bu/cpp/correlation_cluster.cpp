...
void read_table(string fn,
		vector< vector<double> >& table, bool to_rank,
...
int main(int argc, char **argv)
{
  ...
  read_table(ifn, table, method == mSpearman, map_back, N, M);
  ...
  for (int i=0; i<N; i++) {
    ...
    if (final_index_map.find(cluster_idx) == final_index_map.end())
      final_index_map[cluster_idx] = final_index_map.size() + 1;
    int index = final_index_map[cluster_idx];
    out << map_back[i] << "\t" << index << endl;
