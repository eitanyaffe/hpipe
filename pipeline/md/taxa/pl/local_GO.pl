...
if ($#ARGV == -1) {
    print STDERR "usage: $0 <set taxa reps> <gene set table> <set field> <uniref top hits> <taxa nodes> <taxa names> <taxa merged> <taxa deleted> <min identity> <min gene cover> <mask cag T|F> <mask ids> <ofn set summary> <ofn taxa table> <ofn trees>\n";
	exit 1;
}
...
while (my $line = <IN>) {
	...
	$sets{$set}->{count}->{weak} = 0;
	$sets{$set}->{count}->{hit} = 0;
...
print OUT "anchor\tno_hit\tno_taxa\tmasked\tweak_hit\thit\n";
foreach my $set (sort {$a <=> $b} keys %sets) {
    my $no_hit = $sets{$set}->{count}->{no_hit};
    my $no_taxa = $sets{$set}->{count}->{no_taxa};
    my $weak = $sets{$set}->{count}->{weak};
    my $hit = $sets{$set}->{count}->{hit};
    my $masked = $sets{$set}->{count}->{masked};
    print OUT $set, "\t", $no_hit, "\t", $no_taxa, "\t", $masked, "\t", $weak, "\t", $hit, "\n";
